...
- `src/GoogleButton.tsx` — "Continue with Google" button, rendered by both `login.tsx` and `signup.tsx` (Google handles sign-in and first-time sign-up alike). `POST`s `/api/auth/sign-in/social` (`{ provider: 'google', callbackURL: '/api/auth/oauth2/authorize?<oauth_query>' }`) and redirects the browser to the returned `url`; after Google's callback Better Auth sets the session cookie and lands on `authorize`, resuming the OAuth flow the same way the email sign-in does.

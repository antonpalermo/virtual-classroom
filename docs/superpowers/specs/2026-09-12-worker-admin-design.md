...
- **Add the `admin` plugin** (`better-auth/plugins/admin`) to `auth.ts`:
  - `adminRoles: ['admin']` — matches the existing placeholder `role` field's intended values.
...
- `package.json`: `@capstone/admin`, `private: true`, `version: "0.0.0"` (bumped via Changesets once real work lands). Dependencies: `hono`, `@capstone/standards` (HTTP status constants, same as `worker-realtime`). Dev dependencies: `@capstone/typescript`, `@cloudflare/workers-types`, `wrangler`.
